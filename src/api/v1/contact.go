...
func HandleContact() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("HX-Request") == "true" {
			html.ContactView(languageContent, theme).Render(r.Context(), w)
			return
		}

		html.Document(languageContent, theme, html.ContactView(languageContent, theme)).Render(r.Context(), w)
	}
}
